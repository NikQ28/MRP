...
import type { Item, Stock, OperationType, StockStateRow } from '../types'
...
export function useStocksPage() {
  ...
  const [snapshotRows, setSnapshotRows] = useState<StockStateRow[] | null>(null)
  ...
  const onCalculateSnapshot = useCallback(async () => {
    setSnapshotLoading(true)
    setSnapshotError(null)
    try {
      const rows = await stocksApi.getCurrentStocks(asOfDate)
      setSnapshotRows(rows)
    } catch (e) {
      setSnapshotError(e instanceof Error ? e.message : 'Ошибка запроса')

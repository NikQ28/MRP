    ...
    public class CalculateService(IBomRepository bomRepository, IStockRepository stockRepository, IOrderRepository orderRepository, ITreeService treeService) : ICalculateService
    {
        public async Task<List<RequiredItem>> Calculate(DateTime datetime)
        {
            ...
            var orders = (await orderRepository.GetAsync())
                ...
                .Where(o => o.Execution.Date >= datetime.Date)
                .Where(o => o.Creation.Date <= datetime.Date)
                .ToList();
            ...
            var stocks = (await stockRepository.Get())
                ...
                .GroupBy(s => s.ItemId)
                .ToDictionary(g => g.Key, g => Math.Max(0, g.Sum(s => s.Operation == OperationType.Coming ? s.Count : -s.Count)));
            ...
            foreach (var orderString in orderStrings)
                CalculateItem(orderString.ItemId, orderString.Count, required, stocks, children);
        ...
        private void CalculateItem(int itemId, int requiredCount, List<RequiredItem> required, Dictionary<int, int> stocks, Dictionary<int, List<Bom>> children)
        {
            ...
            stocks[itemId] = stocks.ContainsKey(itemId) ? stocks[itemId] : 0;
            var takeFromStocks = Math.Min(requiredCount, stocks[itemId]);

            requiredItem.StockCount += takeFromStocks;
            stocks[itemId] -= takeFromStocks;
            ...
            if (children.TryGetValue(itemId, out var boms))
                foreach (var bom in boms)
                    CalculateItem(bom.ComponentId, needToProduce * bom.Count, required, stocks, children);
        }
    }
}